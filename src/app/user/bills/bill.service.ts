...
export class BillService {
    ...
    getAllByRole(osbbRole: string, userId: number, searchDTO: SearchDTO, status: string): Observable<any> {
        if (osbbRole != 'HEAD') {
            ...
            return this._http.post(this.billsURL + '/?status=' + status, JSON.stringify(searchDTO), {headers: this.headers})
    ...
    getAllByUser(userId: number, searchDTO: SearchDTO, status: string): Observable<any> {
        return this._http.post(this.userBillsURL + userId + '/all?status=' + status, JSON.stringify(searchDTO), {headers: this.headers})
            .map((response)=> response.json())
            .catch((error)=>Observable.throw(error));
    }
    ...
    saveBill(bill: BillDTO): Observable<any> {
        return this._http.post(this.billsURL + '/save', JSON.stringify(bill), {headers: this.headers})
            .catch((error)=>Observable.throw(error));
    }

    update(bill: BillDTO): Observable<any> {
        return this._http.put(this.billsURL, JSON.stringify(bill), {headers: this.headers})
            .catch((error)=>Observable.throw(error));
    }


    deleteById(billId: number): Observable<any> {
        return this._http.delete(this.billsURL + '/' + billId, {headers: this.headers})
            .catch((error)=>Observable.throw(error))
    }
